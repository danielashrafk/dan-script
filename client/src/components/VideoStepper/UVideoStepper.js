...
import{FormControl, Select, InputLabel, FormHelperText, MenuItem, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Container, Grow, Grid, Typography, createMuiTheme, ThemeProvider, Button, Stepper, Step, StepLabel, StepContent, Paper, StepConnector, withStyles, makeStyles, LinearProgress, IconButton  } from '@material-ui/core';
...
import {videoToAudio, optimiseVideo, transcribeVideo, youtubeToAudio, optimiseYoutube, checkVideo} from '../../actions/files.js'
...
const VideoStepper = ({youtubeURL, setResponse, handleClick, language, channel, videoStepper}) => {
    ...
    const columns = ["Word", "Time", {
      ...
      options: {
        ...
        customBodyRender: (value) => {
          return (
            <a href={value}><i className="fas fa-link"></i></a>
          );
        }
      }
    },];
    ...
    async function handleStart() {
      ...
      var videoData = null;
      videoData = await dispatch(checkVideo({youtubeURL}));
      // console.log(videoData);
      if(videoData !== "Could not find video"){
        var i;
        for(i = 0; i< videoData['transcription'].length; i++){
          let a = tableData;
          a.push([videoData['transcription'][i], videoData['timing2'][i], `${youtubeURL}&t=${videoData['timing'][i]}s`]);
          setTableData(a);
      ...
      var youtubeToAudioData = null;
      youtubeToAudioData = await dispatch(youtubeToAudio({youtubeURL}));
      console.log(youtubeToAudioData);
      if(youtubeToAudioData === "Something went wrong") {
        ...
        setActiveStep(1);
        optimiseVideoData = await dispatch(optimiseYoutube({youtubeToAudioData}));
        ...
        if(optimiseVideoData === 'Something went wrong.') {
          ...
          transcribeVideoData = await dispatch(transcribeVideo({optimiseVideoData, language, channel, youtubeURL}));
          ...
          if(transcribeVideoData === 'Something went wrong.') {
            ...
            for(i = 0; i< transcribeVideoData[0].length; i++){
              let a = tableData;
              a.push([transcribeVideoData[0][i], transcribeVideoData[2][i], `${youtubeURL}&t=${transcribeVideoData[1][i]}s`]);
              setTableData(a);
      ...
      <div className={classes.root}>
            ...
            {activeStep === steps.length ? (
              <>
                ...
                <Typography className= "gradient" style={{fontSize: "20px"}}>Search</Typography>
                <i className="gradient fab fa-searchengin" style={{paddingLeft:"1vw"}}></i>
                </Grid>

                <Grid item container xs={12} justify="center" style={{paddingTop: "20px", paddingBottom: "20px"}}>
                 <SearchBar
